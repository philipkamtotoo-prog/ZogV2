...
- 当前实机金币来源包括挂机、乞讨、赌博获胜和节目结算奖励；客厅作弊按钮仅为开发工具，后续删除。
- 演员好感只来自下注支持和演员送礼。
...
## 17. 设计真源与功能状态总表（2026-05-12）

> 2026-05-12 更新：4/30 的状态表已过时，当前实机状态以 `V2_当前实机对齐扫描_0512.md` 和 `待办.md` 为准。下方原有 4/30 表格只保留为历史施工记录，不再作为当前差异判断来源。

当前已确认的实机口径：

- 场景、首发演员、伤害公式、行动表、FinalScore、MVP、赔率、挂机、乞讨、设备升级、演员片酬、节目变异液、Pixi 基建等已经完成过一轮对齐。
- 金币来源以当前代码为准：挂机、乞讨、赌博获胜、结算观看奖励、完整节目奖励。
- BYOK 已经升级为 `LLMRoleConfigMap`，当前按功能区 role 配置模型，不是每个演员单独配置模型。
- 未完成项统一进入 `待办.md`，避免在开发计划、扫描报告、设计文档里重复维护多份冲突清单。
